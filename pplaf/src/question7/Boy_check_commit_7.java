 ...
 * @author sony
 */
public class Boy_check_commit_7 {
    ...
    public void check1(Boy b_check[],Girl g_check[],int no_b,int no_g){
         array com=new array();
         binarySearch com1=new binarySearch();
         hashtable com2=new hashtable();
         int no_of_couple=0;
         ...
         for(int i=0;i<no_g;i++){
             for(int j=0;j<no_b;j++){
                 if((b_check[j].budget>=g_check[i].budget) &&(b_check[j].min_attract<=g_check[i].attractiveness)&&(b_check[j].status==0)){
                     ...
                     com1.bg_commited(b_check[j],g_check[i],no_of_couple);
                     com2.bg_commited(b_check[j],g_check[i],no_of_couple);
                     //com_g[j]=1;
